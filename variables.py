...
# Each group is (heading, [(english_column_key, human_readable_label), ...]).
DASHBOARD_VARIABLE_GROUPS: list[tuple[str, list[tuple[str, str]]]] = [
    ("Demographics", [
...
# Flattened for existing consumers (geo_builder.py etc.) that expect a
# plain list of (key, label) pairs — keeps them working unchanged.
DASHBOARD_VARIABLES: list[tuple[str, str]] = [
    pair for _, group in DASHBOARD_VARIABLE_GROUPS for pair in group
]